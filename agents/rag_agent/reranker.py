...
class Reranker:
    ...
    def __init__(self, config):
        ...
        try:
            ...
            self.logger.warning(
                f"Reranker unavailable ({e}). Falling back to retrieval-only mode."
            )
            self.model = None
            self.top_k = config.rag.reranker_top_k
    ...
    def rerank(self, query: str, documents: Union[List[Dict[str, Any]], List[str]], parsed_content_dir: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # If reranker model is unavailable (e.g., offline environment),
            # gracefully fall back to original retrieval ordering.
            if self.model is None:
                self.logger.info("Reranker model is unavailable, using retrieval-only ranking")
                fallback_docs = documents
                if self.top_k and len(fallback_docs) > self.top_k:
                    fallback_docs = fallback_docs[:self.top_k]
